...
\begin{frame}{Fonctionnement}{Preuve de travail}
	\only<1>{
	\begin{center}
		\includegraphics[scale=0.2]{mining.png}
	\end{center}
	\begin{center}
		La preuve de travail(\textbf{proof-of-work)} est un algorithme de consensus dans un réseau de Blockchain. Cet algorithme est utilisé pour confirmer les transactions et produire des nouveaux blocks dans la chaine en prouvant que des calculs informatique onéreux ont été fait.
	\end{center}
	\begin{center}
		\begin{itemize}
			\item Utilisé par la plupart des crypto-monnaies(BTC,ETH,...)
			\item Les transactions du block ajouté sont valides(\color{blue}{pourquoi?}
			)
		\end{itemize}
	\end{center}}
	...
	\begin{center}
		\begin{itemize}
			...
			\item Plus il y a de la puissance informatique ajoutée dans le réseau, plus dur seront les puzzle
		\end{itemize}
	\end{center}}
\end{frame}
